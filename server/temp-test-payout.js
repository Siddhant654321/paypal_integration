"use strict";
var import_seller_payments = require("./seller-payments");
...
async function testPayPalPayout() {
  try {
    ...
    const testData = {
      ...
      sellerId: 1,
      amount: 1e3,
      // $10.00 in cents - using a smaller amount for testing
    ...
    const result = await import_seller_payments.SellerPaymentService.createPayout(
      testData.paymentId,
      testData.sellerId,
      testData.amount,
    ...
    await import_storage.storage.createSellerPayOut({
      ...
      completedAt: result.batch_header.batch_status === "SUCCESS" ? /* @__PURE__ */ new Date() : null
    });
...
testPayPalPayout().then(() => console.log("[PAYPAL] Test completed successfully")).catch((error) => console.error("[PAYPAL] Test failed:", error)).finally(() => process.exit());

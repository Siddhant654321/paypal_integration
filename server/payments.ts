...
const SELLER_FEE_PERCENTAGE = 0.1;
const INSURANCE_FEE = 800;

export class PaymentService {
  private static logPaymentFlow(
    orderId: string,
    status: string,
    details?: any,
  ) {
  ...
  static async createCheckoutSession(
    ...
    try {
      ...
      const sellerPayout =
        baseAmount - Math.round(baseAmount * SELLER_FEE_PERCENTAGE);
      ...
      const orderRequest = {
        ...
        application_context: {
          ...
          user_action: "PAY_NOW",
          return_url: `${process.env.APP_URL || "http://localhost:5001"}/payment/success`,
          cancel_url: `${process.env.APP_URL || "http://localhost:5001"}/payment/cancel`,
        },
      };
      ...
      try {
        ...
        if (error.response?.data?.details) {
          ...
          throw new Error(
            `PayPal Error: ${details.map((d) => d.description || d.issue).join(", ")}`,
          );
        }
          ...
          payment_source: {
            paypal: {
              ...
              experience_context: {
                ...
                user_action: "PAY_NOW",
                return_url: `https://c02c53b2-ad62-4250-ae57-133efc790f6e-00-oipuw3nyja7q.spock.repl.co/payment/success`,
                cancel_url: `https://c02c53b2-ad62-4250-ae57-133efc790f6e-00-oipuw3nyja7q.spock.repl.co/payment/cancel`,
              },
            },
          },
        },
  ...
  static async authorizeOrder(orderId: string) {
    try {
      ...
      if (
        orderDetails.data.status !== "APPROVED" &&
        orderDetails.data.status !== "COMPLETED"
      ) {
        throw new Error(
          `Invalid order status for authorization: ${orderDetails.data.status}. Order must be approved first.`,
        );
      }
      ...
      if (orderDetails.data.intent !== "AUTHORIZE") {
        throw new Error(
          `Invalid order intent: ${orderDetails.data.intent}. Expected: AUTHORIZE`,
        );
      }
      ...
      if (
        !response.data?.purchase_units?.[0]?.payments?.authorizations?.[0]?.id
      ) {
  ...
  static async captureAuthorizedPayment(
    ...
    try {
      ...
      if (
        authStatus.data.status !== "CREATED" &&
        authStatus.data.status !== "AUTHORIZED"
      ) {
        throw new Error(
          `Invalid authorization status: ${authStatus.data.status}`,
        );
      }
  ...
  static async handlePaymentSuccess(orderId: string) {
    try {
      ...
      if (
        ...
        if (payment) {
          ...
          await NotificationService.createNotification({
            ...
            message:
              "Please review and approve your purchase to complete payment.",
            metadata: { orderId },
          });
        }
      ...
      while (attempts < maxAttempts) {
        try {
          console.log(
            `[PAYPAL] Attempting to capture payment (attempt ${attempts + 1}/${maxAttempts})`,
          );
            ...
            {},
            {
              headers: {
                ...
                "PayPal-Partner-Attribution-Id":
                  process.env.PAYPAL_BN_CODE || "AgriMarketplace_SP",
                Prefer: "return=representation",
              },
          ...
          lastError = error;
          console.error(
            `[PAYPAL] Capture attempt ${attempts} failed:`,
      ...
      if (attempts === maxAttempts) {
        throw new Error(
          `Failed to capture payment after ${maxAttempts} attempts: ${lastError?.message}`,
        );
      }
  ...
  static async getOrderStatus(orderId: string) {
    ...
    while (attempts < maxAttempts) {
      try {
        console.log(
          `[PAYPAL] Getting order status for ${orderId} (attempt ${attempts + 1}/${maxAttempts})`,
        );
        const accessToken = await this.getAccessToken();
        ...
        attempts++;
        console.error(
          `[PAYPAL] Error getting order status (attempt ${attempts}/${maxAttempts}):`,
          error,
        );
        ...
        if (attempts === maxAttempts) {
          throw new Error(
            `Failed to get order status after ${maxAttempts} attempts: ${error.message}`,
          );
        }
  ...
  static async handlePaymentFailure(orderId: string) {
    try {
      ...
      this.logPaymentFlow(orderId, "PAYMENT_STATUS_UPDATED", {
        status: "failed",
      });
      ...
      this.logPaymentFlow(orderId, "AUCTION_STATUS_UPDATED", {
        status: "failed",
      });
      ...
      this.logPaymentFlow(orderId, "NOTIFICATION_SENT", {
        userId: payment.sellerId,
        type: "payment",
        title: "Payment Failed",
      });
      ...
      this.logPaymentFlow(orderId, "PAYMENT_FAILURE_ERROR", {
        error: error.message,
      });
      throw error;
    }
  ...
  static async releaseFundsToSeller(paymentId: number, trackingInfo: string) {
    try {
      ...
      this.logPaymentFlow(String(paymentId), "PAYMENT_RECORD_FOUND", {
        payment,
      });

      if (payment.status !== "completed_pending_shipment") {
        throw new Error(
          `Invalid payment status for fund release: ${payment.status}`,
        );
      }
      ...
      const sellerPayout =
        payment.sellerPayout || Math.round(payment.amount * 0.9);
      ...
      if (
        ...
        console.warn(
          "[PAYPAL] PayPal not fully configured, using simulated payout",
        );
        ...
        payoutResult = await SellerPaymentService.createPayout(
          ...
          sellerPayout,
          sellerProfile?.paypalMerchantId ||
            `test-${payment.sellerId}@example.com`,
        );
      } else {
      ...
      if (
        !payoutResult ||
        (!payoutResult.batch_header?.payout_batch_id && !payoutResult.simulated)
      ) {
      ...
      console.log(
        "[PAYPAL] Payout successful, updating payment status to completed",
      );
      ...
      this.logPaymentFlow(String(paymentId), "PAYMENT_STATUS_UPDATED", {
        status: "completed",
      });
      ...
      this.logPaymentFlow(String(paymentId), "AUCTION_STATUS_UPDATED", {
        status: "fulfilled",
      });
      ...
      this.logPaymentFlow(String(paymentId), "NOTIFICATION_SENT", {
        userId: payment.buyerId,
        type: "fulfillment",
      });
      ...
      this.logPaymentFlow(String(paymentId), "FUND_RELEASE_ERROR", {
        error: error.message,
      });
      throw error;
    }
  ...
  static async getPaymentStatus(auctionId: number): Promise<string> {
    try {
      ...
      this.logPaymentFlow(String(auctionId), "PAYMENT_RECORD_FOUND", {
        payment,
      });
      ...
      this.logPaymentFlow(String(auctionId), "PAYMENT_STATUS_ERROR", {
        error: error.message,
      });
    ...
    try {
      ...
      const payoutRequest = {
        sender_batch_header: {
          ...
          email_message:
            "Your auction payment has been processed and funds are now available.",
        },
        items: [
          {
      ...
      await storage.createSellerPayOut({
        ...
        createdAt: new Date(),
        completedAt:
          response.data.batch_header.batch_status === "SUCCESS"
            ? new Date()
            : null,
      });
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        if (error.response.data?.name === "VALIDATION_ERROR") {
          throw new Error(
            `PayPal validation error: ${error.response.data?.message || "Invalid payout request"}`,
          );
        }
      }
      throw new Error(
        "Failed to process seller payout. Please ensure your PayPal account is properly configured.",
      );
    }
  }
}

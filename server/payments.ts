...
export class PaymentService {
    ...
    static async createOrder(auctionId: number, buyerId: number, includeInsurance: boolean = false) {
        try {
            console.log("[PAYPAL] Creating order for auction:", { auctionId, buyerId });
            ...
            const amountUSD = (totalAmount / 100).toFixed(2);
            const baseAmountUSD = (baseAmount / 100).toFixed(2);
            const feesUSD = ((platformFee + insuranceFee) / 100).toFixed(2);
            ...
            console.log("[PAYPAL] Order created successfully:", { orderId, status: response.data.status });
    ...
    static async capturePayment(orderId: string): Promise<boolean> {
        try {
            ...
            if (orderStatus.status !== 'APPROVED') {
                throw new Error(`Order not ready for capture. Status: ${orderStatus.status}`);
            }
            ...
            if (response.data.status !== 'COMPLETED') {
                throw new Error(`Capture failed. Status: ${response.data.status}`);
            }

...
import { CheckCircle2, Loader2, PencilIcon, Trash2, Edit } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState("pending-auctions");

  // Set active tab based on URL hash or query param
  useEffect(() => {
  ...
  // Queries for different auction types
  const pendingAuctionsQuery = useQuery({
    queryKey: ["/api/admin/auctions", { approved: false }],
    queryFn: () => apiRequest("GET", "/api/admin/auctions?approved=false"),
  });

  const approvedAuctionsQuery = useQuery({
    queryKey: ["/api/admin/auctions", { approved: true }],
    queryFn: () => apiRequest("GET", "/api/admin/auctions?approved=true"),
  });

  // Approve auction mutation
  const approveAuctionMutation = useMutation({
    mutationFn: (auctionId: number) => 
      apiRequest("POST", `/api/admin/auctions/${auctionId}/approve`),
    onSuccess: () => {
      toast({
        title: "Auction approved",
        description: "The auction has been approved successfully.",
      });
      // Invalidate both auction queries to refresh the data
      queryClient.invalidateQueries({ queryKey: ["/api/admin/auctions"] });
    },
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to approve auction. " + (error as Error).message,
        variant: "destructive",
      });
    },
  });

  // Delete auction mutation - THIS WAS MISSING OR INCORRECTLY IMPLEMENTED
  const deleteAuctionMutation = useMutation({
    mutationFn: (auctionId: number) => 
      apiRequest("DELETE", `/api/admin/auctions/${auctionId}`),
    onSuccess: () => {
      toast({
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to delete auction. " + (error as Error).message,
        variant: "destructive",
      });
    },
  });
    ...
    <div className="container mx-auto p-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage auctions and user accounts</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="mb-4">
              <TabsTrigger value="pending-auctions">Pending Auctions</TabsTrigger>
              <TabsTrigger value="approved-auctions">Approved Auctions</TabsTrigger>
              <TabsTrigger value="users">Users</TabsTrigger>
              <TabsTrigger value="settings">Settings</TabsTrigger>
            </TabsList>

            <TabsContent value="pending-auctions">
              <div className="space-y-4">
                <h2 className="text-xl font-bold">Pending Auctions</h2>
                {pendingAuctionsQuery.isPending ? (
                  <div className="flex justify-center p-8">
                    <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
                  </div>
                ) : pendingAuctionsQuery.isError ? (
                  <div className="text-center text-destructive p-8">
                    Error loading auctions: {(pendingAuctionsQuery.error as Error).message}
                  </div>
                ) : pendingAuctionsQuery.data.length === 0 ? (
                  <div className="text-center text-muted-foreground p-8">
                    No pending auctions found.
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {pendingAuctionsQuery.data.map((auction: any) => {
                      return (
                        <AuctionCard
                          key={auction.id}
                          auction={auction}
            ...
            <TabsContent value="approved-auctions">
              <div className="space-y-4">
                <h2 className="text-xl font-bold">Approved Auctions</h2>
                {approvedAuctionsQuery.isPending ? (
                  <div className="flex justify-center p-8">
                    <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
                  </div>
                ) : approvedAuctionsQuery.isError ? (
                  <div className="text-center text-destructive p-8">
                    Error loading auctions: {(approvedAuctionsQuery.error as Error).message}
                  </div>
                ) : approvedAuctionsQuery.data.length === 0 ? (
                  <div className="text-center text-muted-foreground p-8">
                    No approved auctions found.
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {approvedAuctionsQuery.data.map((auction: any) => (
                      <AuctionCard
                        key={auction.id}
                        auction={auction}
                        showStatus
